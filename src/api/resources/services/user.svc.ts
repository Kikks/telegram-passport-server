...
const createUser = async (userDetails: IUser) => {
  ...
  if (existingUser) throw new ootpError('A user with that email already exists.', 409);
...
const updateUser = async ({ query, userDetails }: { query: any; userDetails: Partial<OUser> }) => {
  ...
  if (!user) throw new ootpError('No user with that id exists.', 404);
...
const registerAdmin = async ({ userDetails, password }: RegisterUserParams) => {
  const existingUser = await getUser({ email: userDetails?.email });
  if (existingUser) throw new ootpError('A user with that email already exists.', 409);
